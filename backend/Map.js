...
export class MapFactory{
    static map1(){
        let m = new Map();
        m.lasers.push(new Laser([Map.xMin + 0.2 * (Map.xMax - Map.xMin), Map.yMin + 0.3 * (Map.yMax - Map.yMin)],1,1000,0.007, 300, false, 0, null))
        m.lasers.push(new Laser([Map.xMin + 0.2 * (Map.xMax - Map.xMin), Map.yMin + 0.7 * (Map.yMax - Map.yMin)],2,1000,0.007, 300, false, 0, null))
        m.lasers.push(new Laser([Map.xMin + 0.4 * (Map.xMax - Map.xMin), Map.yMin + 0.3 * (Map.yMax - Map.yMin)],3,1000,0.007, 300, false, 0, null))
